...
extension Modules {
    struct App: Cleanse.Module {
        static func configure(binder: AppBinder) {
            ...
            binder.bind().to(factory: LoginPresenterFactory.init)
            binder.bind(LoginViewController.Router.self).tagged(with: LoginRouter.self).to(factory: LoginRouter.init)
            ...
            binder.bind().to(factory: OnboardingPresenterFactory.init)
            binder.bind(OnboardingViewController.Router.self).tagged(with: OnboardingRouter.self).to(factory: OnboardingRouter.init)
